...
export default function DattuYojana() {
  ...
  async function loadData() {
    ...
    try {
      ...
      // 🔥 FIX: Handle Wrapper
      let dataList = [];
      if (dattuData && dattuData.data && Array.isArray(dattuData.data)) {
          dataList = dattuData.data;
      } else if (Array.isArray(dattuData)) {
          dataList = dattuData;
      }
      setRows(dataList);

      const cattleRes = await fetchCattle();
      let cattleList = [];
      if (cattleRes && cattleRes.data && Array.isArray(cattleRes.data)) cattleList = cattleRes.data;
      else if (Array.isArray(cattleRes)) cattleList = cattleRes;
      ...
      {selectedEntry && (
        <div style={overlayStyle} onClick={() => setSelectedEntry(null)}>
           <div style={viewModalStyle} onClick={e => e.stopPropagation()}>
              ...
              <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))", gap:"1.2rem", fontSize:"0.9rem"}}>
                 ...
                 <div style={{gridColumn:"1 / -1", borderTop:"1px dashed #eee", margin:"5px 0"}}></div>
...
function DetailItem({ label, value, isBold, fullWidth }) { return <div style={{ gridColumn: fullWidth ? "1 / -1" : "auto" }}><div style={{fontSize:"0.75rem", color:"#6b7280", textTransform:"uppercase", letterSpacing:"0.02em"}}>{label}</div><div style={{fontWeight: isBold ? "700" : "500", color: "#111827", marginTop:"2px"}}>{value || "-"}</div></div>; }