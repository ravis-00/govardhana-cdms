...
export default function Breeds() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      // Fetch specifically for "breeds"
      const res = await fetchMaster("breeds");
  ...
  function getEmptyForm() {
    return { id: "", breed_name: "", origin: "Cow", description: "" };
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await updateMaster("breeds", form.id, form);
      } else {
        await addMaster("breeds", form);
      }
      setShowModal(false);
      loadData();
    } catch (err) {
  ...
  return (
    <div style={{ padding: "1.5rem", maxWidth: "1200px", margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
        <h1 style={{ fontSize: "1.6rem", fontWeight: "700", color: "#1f2937", margin: 0 }}>Breed Configuration</h1>
        ...
        <div style={{ overflowX: "auto" }}> {/* 🔥 SCROLLABLE */}
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.9rem", minWidth: "600px" }}>
            <thead style={{ background: "#f9fafb", borderBottom: "2px solid #e5e7eb" }}>
              <tr>
                <th style={thStyle}>Breed Name</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={4} style={{ padding: "2rem", textAlign: "center", color: "#9ca3af" }}>No breeds found.</td></tr>
              ) : (
                rows.map((row, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #f3f4f6" }}>
                    <td style={tdStyle}><strong>{row.breed_name}</strong></td>
                    <td style={tdStyle}>
                      <span style={{ 
            ...
            <form onSubmit={handleSubmit} style={{ display: "grid", gap: "1rem" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                <div>
                  ...
                  <input type="text" name="breed_name" value={form.breed_name} onChange={handleChange} style={inputStyle} required />
                </div>
                <div>
                  <label style={labelStyle}>Origin (Type)</label>
                  <select name="origin" value={form.origin} onChange={handleChange} style={inputStyle}>
              ...
              <div>
                ...
                <textarea name="description" rows="3" value={form.description} onChange={handleChange} style={inputStyle} placeholder="e.g. Draught breed from Karnataka" />
              </div>
